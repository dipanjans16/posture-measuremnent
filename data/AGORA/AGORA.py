...
class AGORA(torch.utils.data.Dataset):
    def __init__(self, transform, data_split):
        ...
        self.resolution = (
            2160,
            3840,
        )  # height, width. one of (720, 1280) and (2160, 3840)
        ...
        self.joint_set = {
            ...
            "joints_name": (
                ...
                "L_Middle_4",
                "L_Ring_4",
                "L_Pinky_4",  # finger tips
                "R_Thumb_4",
                "R_Index_4",
                "R_Middle_4",
                "R_Ring_4",
                "R_Pinky_4",  # finger tips
        ...
        self.joint_set["joint_part"] = {
            "body": list(
                range(
                    self.joint_set["joints_name"].index("Pelvis"),
                    self.joint_set["joints_name"].index("R_Eye_SMPLH") + 1,
                )
            )
            + list(
                range(
                    self.joint_set["joints_name"].index("Nose"),
                    self.joint_set["joints_name"].index("R_Heel") + 1,
                )
            ),
            "lhand": list(
                range(
                    self.joint_set["joints_name"].index("L_Index_1"),
                    self.joint_set["joints_name"].index("L_Thumb_3") + 1,
                )
            )
            + list(
                range(
                    self.joint_set["joints_name"].index("L_Thumb_4"),
                    self.joint_set["joints_name"].index("L_Pinky_4") + 1,
                )
            ),
            "rhand": list(
                range(
                    self.joint_set["joints_name"].index("R_Index_1"),
                    self.joint_set["joints_name"].index("R_Thumb_3") + 1,
                )
            )
            + list(
                range(
                    self.joint_set["joints_name"].index("R_Thumb_4"),
                    self.joint_set["joints_name"].index("R_Pinky_4") + 1,
                )
            ),
            "face": list(
                range(
                    self.joint_set["joints_name"].index("Face_5"),
    ...
    def load_data(self):
        datalist = []
        if self.data_split == "train" or (
            self.data_split == "test" and self.test_set == "val"
        ):
            ...
            for aid in db.anns.keys():
                ...
                if self.resolution == (720, 1280):
                    ...
                    2160,
                    3840,
                ):  # use cropped and resized images. loading 4K images in pytorch dataloader takes too much time...
                    img_path = osp.join(
                        self.data_path,
                        "3840x2160",
                        img["file_name_3840x2160"].split("/")[-2] + "_crop",
                        img["file_name_3840x2160"].split("/")[-1][:-4]
                        + "_ann_id_"
                        + str(aid)
                        + ".png",
                    )
                    ...
                    with open(json_path) as f:
                        ...
                        img2bb_trans_from_orig = np.array(
                            crop_resize_info["img2bb_trans"], dtype=np.float32
                        )
                        resized_height, resized_width = (
                            crop_resize_info["resized_height"],
                            crop_resize_info["resized_width"],
                        )
                    img_shape = (resized_height, resized_width)
                    bbox = np.array(
                        [0, 0, resized_width, resized_height], dtype=np.float32
                    )
                    ...
                    lhand_bbox = np.dot(
                        img2bb_trans_from_orig,
                        np.concatenate(
                            (lhand_bbox, np.ones_like(lhand_bbox[:, :1])), 1
                        ).transpose(1, 0),
                    ).transpose(1, 0)
                    lhand_bbox[1] -= lhand_bbox[0]  # xyxy -> xywh
                    lhand_bbox = lhand_bbox.reshape(4)
                    lhand_bbox = sanitize_bbox(
                        lhand_bbox, self.resolution[1], self.resolution[0]
                    )
                    if lhand_bbox is not None:
                        lhand_bbox[2:] += lhand_bbox[:2]  # xywh -> xyxy
                    ...
                    rhand_bbox = np.dot(
                        img2bb_trans_from_orig,
                        np.concatenate(
                            (rhand_bbox, np.ones_like(rhand_bbox[:, :1])), 1
                        ).transpose(1, 0),
                    ).transpose(1, 0)
                    rhand_bbox[1] -= rhand_bbox[0]  # xyxy -> xywh
                    rhand_bbox = rhand_bbox.reshape(4)
                    rhand_bbox = sanitize_bbox(
                        rhand_bbox, self.resolution[1], self.resolution[0]
                    )
                    if rhand_bbox is not None:
                        rhand_bbox[2:] += rhand_bbox[:2]  # xywh -> xyxy
                    ...
                    face_bbox = np.dot(
                        img2bb_trans_from_orig,
                        np.concatenate(
                            (face_bbox, np.ones_like(face_bbox[:, :1])), 1
                        ).transpose(1, 0),
                    ).transpose(1, 0)
                    face_bbox[1] -= face_bbox[0]  # xyxy -> xywh
                    face_bbox = face_bbox.reshape(4)
                    face_bbox = sanitize_bbox(
                        face_bbox, self.resolution[1], self.resolution[0]
                    )
                    if face_bbox is not None:
                        face_bbox[2:] += face_bbox[:2]  # xywh -> xyxy
        ...
        if self.data_split == "train":
            ...
            with open(data["joints_2d_path"]) as f:
                ...
                if self.resolution == (2160, 3840):
                    joint_img[:, :2] = np.dot(
                        data["img2bb_trans_from_orig"],
                        np.concatenate(
                            (joint_img, np.ones_like(joint_img[:, :1])), 1
                        ).transpose(1, 0),
                    ).transpose(
                        1, 0
                    )  # transform from original image to crop_and_resize image
            ...
            lhand_bbox, lhand_bbox_valid = self.process_hand_face_bbox(
                lhand_bbox, do_flip, img_shape, img2bb_trans
            )
            rhand_bbox, rhand_bbox_valid = self.process_hand_face_bbox(
                rhand_bbox, do_flip, img_shape, img2bb_trans
            )
            face_bbox, face_bbox_valid = self.process_hand_face_bbox(
                face_bbox, do_flip, img_shape, img2bb_trans
            )
            if do_flip:
            ...
            joint_img[self.joint_set["joint_part"]["body"], 2] = (
                (
                    joint_cam[self.joint_set["joint_part"]["body"], 2].copy()
                    / (cfg.body_3d_size / 2)
                    + 1
                )
                / 2.0
            ...
            joint_img[self.joint_set["joint_part"]["lhand"], 2] = (
                (
                    joint_cam[self.joint_set["joint_part"]["lhand"], 2].copy()
                    / (cfg.hand_3d_size / 2)
                    + 1
                )
                / 2.0
            ...
            joint_img[self.joint_set["joint_part"]["rhand"], 2] = (
                (
                    joint_cam[self.joint_set["joint_part"]["rhand"], 2].copy()
                    / (cfg.hand_3d_size / 2)
                    + 1
                )
                / 2.0
            ...
            joint_img[self.joint_set["joint_part"]["face"], 2] = (
                (
                    joint_cam[self.joint_set["joint_part"]["face"], 2].copy()
                    / (cfg.face_3d_size / 2)
                    + 1
                )
                / 2.0
            ...
            # smplx parameters
            root_pose = np.array(
                smplx_param["global_orient"], dtype=np.float32
            ).reshape(
                -1
            )  # rotation to world coordinate
            ...
            lhand_pose = np.array(
                smplx_param["left_hand_pose"], dtype=np.float32
            ).reshape(-1)
            rhand_pose = np.array(
                smplx_param["right_hand_pose"], dtype=np.float32
            ).reshape(-1)
            ...
            trans = np.array(smplx_param["transl"], dtype=np.float32).reshape(
                -1
            )  # translation to world coordinate
            cam_param = {
                "focal": cfg.focal,
                "princpt": cfg.princpt,
            }  # put random camera paraemter as we do not use coordinates from smplx parameters
            smplx_param = {
                "root_pose": root_pose,
                "body_pose": body_pose,
                "shape": shape,
            ...
            smplx_pose_valid[:3] = (
                0  # global orient of the provided parameter is a rotation to world coordinate system. I want camera coordinate system.
            )
            ...
            targets = {
                "joint_img": joint_img,
                "joint_cam": joint_cam,
                "smplx_joint_img": joint_img,
                "smplx_joint_cam": joint_cam,
            ...
            meta_info = {
                ...
                "smplx_joint_trunc": np.zeros_like(joint_trunc),
    ...
    def evaluate(self, outs, cur_sample_idx):
        ...
        for n in range(sample_num):
            ...
            mesh_out_align = (
                mesh_out
                - np.dot(smpl_x.J_regressor, mesh_out)[
            ...
            eval_result["mpvpe_all"].append(
                np.sqrt(np.sum((mesh_out_align - mesh_gt) ** 2, 1)).mean() * 1000
            )
            ...
            mesh_out_lhand_align = (
                mesh_out_lhand
                - np.dot(smpl_x.J_regressor, mesh_out)[
                    smpl_x.J_regressor_idx["lwrist"], None, :
                ]
                + np.dot(smpl_x.J_regressor, mesh_gt)[
                    smpl_x.J_regressor_idx["lwrist"], None, :
                ]
            ...
            mesh_out_rhand_align = (
                mesh_out_rhand
                - np.dot(smpl_x.J_regressor, mesh_out)[
                    smpl_x.J_regressor_idx["rwrist"], None, :
                ]
                + np.dot(smpl_x.J_regressor, mesh_gt)[
                    smpl_x.J_regressor_idx["rwrist"], None, :
                ]
            ...
            eval_result["mpvpe_hand"].append(
                (
                    np.sqrt(
                        np.sum((mesh_out_lhand_align - mesh_gt_lhand) ** 2, 1)
                    ).mean()
                    * 1000
                    + np.sqrt(
                        np.sum((mesh_out_rhand_align - mesh_gt_rhand) ** 2, 1)
                    ).mean()
                    * 1000
                )
                / 2.0
            ...
            eval_result["mpvpe_face"].append(
                np.sqrt(np.sum((mesh_out_face_align - mesh_gt_face) ** 2, 1)).mean()
                * 1000
            )
            ...
            if vis:
                ...
                vis_mesh_out = (
                    vis_mesh_out
                    - np.dot(smpl_x.layer["neutral"].J_regressor, vis_mesh_out)[
                        smpl_x.J_regressor_idx["pelvis"], None, :
                    ]
                )
            ...
            if "person_idx" in annot:
                ...
                exist_result_path = glob(
                    osp.join(cfg.result_dir, "AGORA", save_name + "*")
                )
            ...
            joint_proj = np.dot(
                out["bb2img_trans"], joint_proj.transpose(1, 0)
            ).transpose(1, 0)
            joint_proj[:, 0] = (
                joint_proj[:, 0] / self.resolution[1] * 3840
            )  # restore to original resolution
            joint_proj[:, 1] = (
                joint_proj[:, 1] / self.resolution[0] * 2160
            )  # restore to original resolution
            save_dict = {
                "params": {
    ...
    def print_eval_result(self, eval_result):
        ...
        print(
            "AGORA test results are dumped at: "
            + osp.join(cfg.result_dir, "predictions")
        )

        if (
            self.data_split == "test" and self.test_set == "test"
        ):  # do not print. just submit the results to the official evaluation server
            return
