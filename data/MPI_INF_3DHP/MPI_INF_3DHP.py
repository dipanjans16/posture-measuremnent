...
class MPI_INF_3DHP(torch.utils.data.Dataset):
    ...
    def load_data(self):
        ...
        smplx_param_path = osp.join(
            self.data_path, "MPI-INF-3DHP_SMPLX_NeuralAnnot.json"
        )
        ...
        for aid in db.anns.keys():
            ...
            joint_world = np.array(
                joints[str(subject_idx)][str(seq_idx)][str(frame_idx)], dtype=np.float32
            )
    ...
    def __getitem__(self, idx):
        ...
        img_path, img_shape, bbox, smplx_param, cam_param = (
            data["img_path"],
            data["img_shape"],
            data["bbox"],
            data["smplx_param"],
