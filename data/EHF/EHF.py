...
class EHF(torch.utils.data.Dataset):
    ...
    def load_data(self):
        ...
        for aid in db.anns.keys():
            ...
            mesh_gt_path = osp.join(
                self.data_path, img["file_name"].split("_")[0] + "_align.ply"
            )
    ...
    def __getitem__(self, idx):
        ...
        img_path, img_shape, bbox, mesh_gt_path = (
            data["img_path"],
            data["img_shape"],
            data["bbox"],
            data["mesh_gt_path"],
        ...
        meta_info = {
            ...
            "lhand_bbox_valid": float(True),
            "rhand_bbox_valid": float(True),
    ...
    def evaluate(self, outs, cur_sample_idx):
        ...
        for n in range(sample_num):
            ...
            mesh_gt = np.dot(
                self.cam_param["R"], out["smplx_mesh_cam_target"].transpose(1, 0)
            ).transpose(1, 0)
            ...
            eval_result["pa_mpjpe_body"].append(
                np.sqrt(np.sum((joint_out_body_align - joint_gt_body) ** 2, 1)).mean()
                * 1000
            )
            ...
            eval_result["pa_mpjpe_hand"].append(
                (
                    np.sqrt(
                        np.sum((joint_out_lhand_align - joint_gt_lhand) ** 2, 1)
                    ).mean()
                    * 1000
                    + np.sqrt(
                        np.sum((joint_out_rhand_align - joint_gt_rhand) ** 2, 1)
                    ).mean()
                    * 1000
                )
                / 2.0
            ...
            if vis:
                ...
                joint_img[:, 0] = (
                    joint_img[:, 0] / cfg.output_hm_shape[2] * cfg.input_img_shape[1]
                )
                joint_img[:, 1] = (
                    joint_img[:, 1] / cfg.output_hm_shape[1] * cfg.input_img_shape[0]
                )
                for j in range(len(joint_img)):
                    cv2.circle(
                        img,
                        (int(joint_img[j][0]), int(joint_img[j][1])),
                        3,
                        (0, 0, 255),
                        -1,
                    )
                ...
                cv2.imwrite(
                    os.path.join(kpt_save_folder, str(cur_sample_idx + n) + ".jpg"), img
                )
